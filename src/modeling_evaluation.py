...
def load_data(train_path: str, test_path: str):
    ...
    try:
        ...
        if 'Heart_Disease' not in train_data.columns or 'Heart_Disease' not in test_data.columns:
            raise ValueError("'Heart_Disease' column not found in the dataset")
            
        X_train = train_data.drop(columns=['Heart_Disease'])  # Removed axis parameter
        y_train = train_data['Heart_Disease']
        X_test = test_data.drop(columns=['Heart_Disease'])    # Removed axis parameter
...
def evaluate_model(model, X_test, y_test, class_names, output_dir):
    ...
    try:
        ...
        metrics = {
            ...
            'roc_auc': roc_auc_score(y_test, y_proba[:, 1])  # Use only the positive class probabilities
